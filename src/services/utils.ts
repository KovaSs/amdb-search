...
export const getFBData = (dataName: any): any => app
  .database()
  .ref(dataName)
  .once('value')
  .then((data: any) => data.val())

/** Стили для консольных команд */
export const cloCss: any = { 
...
export const formData = (data: any, formData?: any): any => {
...
export const validateInn = (str: any): boolean => {
  let result = false;
  ...
  let checkDigit = function (str: any, coefficients: any) {
    let n = 0;
...
export const compactObj = (obj: any, clearObj: any = {}): any => {
  filter(obj, (value: any, key: any) => {if(value && value !== "Не найден") clearObj[key] = value})
  return clearObj
}

/** Преобразование первого символа к верхнему регистру, а все остальные к нижнему */
export const toFirstUpper = (str: any): any => str ? `${str[0].toUpperCase()}${str.slice(1).toLowerCase()}` : " "

/* Парсинг ФИО */
export const  parsingFio = (fio: any): any => {
...
export const clearStopLists = (lists: any, alone?: any): any => {
  if(JSON.stringify(lists) === "[]") return []
  else if (alone) {
    lists.rows = lists.rows.map((factorObj: any) => {
      const newFactorObj: any = {}
    ...
    return lists.map((base: any) => {
      base.rows = base.rows.map((factorObj: any) => {
        const newFactorObj: any = {}
        for (const key in factorObj) {
          if (
            factorObj.hasOwnProperty(key) && 
...
export const clearStopListsArr = (lists: any, alone?: any): any => {
  if(JSON.stringify(lists) === "[]") return []
  else if (alone) {
    lists.rows = lists.rows.map((factorObj: any) => {
      const newFactorObj: any = {}
    ...
    lists.map((res: any) => {
      if(!res) return res
        ...
        return res.Response.map((base: any) => {
          if(!base) return base
...
/** Очистка полученных данных по стоп-листам */
export const clearRiskFactors = (factors: any =[], digets?: any): any => {
  if(JSON.stringify(factors) === "[]") return []
  else if (digets) return factors.filter((item: any) =>  moment(item.data_dobavleniya_zapisi).format("L") === moment(Date.now()).format("L") ).map((factorObj: any) => {
    const newFactorObj: any = {}
    for (const key in factorObj) {
      if (
        factorObj.hasOwnProperty(key) && 
  ...
  else return factors.map((factorObj: any) => {
    const newFactorObj: any = {}
    for (const key in factorObj) {
      if (
        factorObj.hasOwnProperty(key) && 
...
/* Парсинг Паспорта */
export const parsingPassport = (passport: any): any => {
...
/* Парсинг адреса */
export const parsingAddress = (address: any): any => {
  let p = address.split(' ');
  let RegionExp: any = ''; // Регион
  let CityExp: any = ''; // Нас. пункт
  let StreetExp: any = ''; // Улица
  let HouseExp: any = ''; // Номер дома
  let BuildExp: any = ''; // Корп
  let BuildingExp: any = ''; // Стр
  let FlatExp: any = ''; // Квар
  let i = p.length - 1;
  if (parseInt(p[i - 3])) {
    FlatExp = p.pop();
    BuildingExp = p.pop();
  ...
  if (p.length) RegionExp = region.filter((item: any) => item.title.toUpperCase().indexOf(p[0].toUpperCase()) !== -1)[0]
...
export const sumTrans = (str: any): any => String(str).replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1' + '\u200A')

/** Форматирование данных арбитража */
export const updateArbiter = (str: any = ""): any => {
...
export const dowloadHtmlFile = (content: any, contentType: any = 'application/octet-stream', filename?: any): void => {
...
export const houseNum = (str: any = ""): any =>  str.match(/\d*/)[0]

/** Генерация Id ключей */
// eslint-disable-next-line
export const uuid = (): any =>  (String(1e7) + -1e3 + -4e3 + -8e3 + -1e11).replace(/[018]/g,(c: any)=>((c as any) ^ crypto.getRandomValues(new Uint8Array(1))[0] & 15 >> (c as any) / 4).toString(16))

/** Преобразование даты к формату DD.MM.YYYY */
export const getDate = (data: any): any =>  moment(data).format('DD.MM.YYYY')

/** Поличение текущей даты в формате DD.MM.YYYY */
export const getNowDate = (): any =>  new Date(Date.now()).toLocaleString().split(", ")[0]

/** Преобразование даты к формату DD.MM.YYYY h:mm:ss */
export const getTimeAndDate = (data: any): any =>  moment(data).format('DD.MM.YYYY HH:mm:ss')

/** Преобразование даты к формату DD.MM.YYYY h:mm:ss */
export const getDownloadTime = (data: any): any =>  moment(data).format('DD.MM.YYYY - HH.mm.ss')

/** Преобразование даты к формату mm:ss */
export const getTime = (data: any): any =>  moment(+data).format('mm:ss')

/** Преобразование даты к формату mm:ss */
export const getTimeDev = (recTime: any, dateNow: any): any =>  {
  ...
  const timeObj: any = {
    ...
    timePercent: moment(dateNow).valueOf() * 100 / moment(+recTime).add(20, 'm').valueOf(),
...
export const getSeconds = (dateNow: any): any => moment(dateNow).seconds() % 2 === 0

/** Преобразование даты к формату YYYY-MM-DD */
export const getUSDate = (arr: any): any => {
  const dateArrUs = arr.map((item: any) => {
...
export const getDatePickerValue = (date: any): any => {
  const dateArr = date.split(".")
    return moment(`${dateArr[2]}-${dateArr[1]}-${dateArr[0]}`)
}

/** Получение объекта даты из строкового значения */
export const getDPValue = (date: any): any => {
  const dateArr = date.split(".")
    return `${dateArr[2]}-${dateArr[1]}-${dateArr[0]}`
}

/** Преобразование имени кампании к укороченной версии */
export const getShortCompName = (name: any): any => {
  return name
...
export const getRegionTitle = (name: any): any => {
  return name && name
    .replace(/^г. /gi, '')
    .replace(/^ГОРОД /gi, '')
}

export const htmlTransform = (str: any): any => {
  return str
...
export const htmlTransformFssp = (str: any): any => {
  return str
...
class TransformData {
  companySource = (inputData: any): any => {
    const cloneFieldsArr = cloneDeep(fieldsArr);
    const orgInfo = cloneFieldsArr.map( (item: any) => {
  ...
  updateComSrc = (prevStore: any, serverData: any, reqnum: any): any => {
    try {
      ...
      if(prevStore.get("leaders_list").length) {
        const historyFl = prevStore.get("leaders_list").map((fl: any) => ({
          id: uuid(),
  ...
  updateIPComSrc = (prevStore: any, serverData: any, reqnum: any): any => {
    try {
      fieldsArrIP.map((fieldItem: any) => {
        for (const key in serverData) {
      ...
      const addHeds = (fio: any, inn: any, date: any): any => {
          const fioArr = fio.split(" ")
  ...
  stop_lists = (prevHeads: any, lists: any = [], id: any): any => {
    return prevHeads = prevHeads.map((head: any) => {
      if(head.id === id) {
        head.timeRequest = Date.now()
        const arr = lists.map((list: any) => {
          if(head.stop_lists && head.stop_lists.filter((item: any) => item.ID_columns === list.ID_columns).length) {
            return null
          } else {
  ...
  getMainDigest = (arr: any, newArr: any = {digets: [], history: []}): any => {
    arr.map((item: any) => {
  ...
  /** Иммутабельное сохранение данных по стоп-листам */
  stopListsArr = (prevHeads: any, lists: any = [], id: any): any => {
    return prevHeads = prevHeads.map((head: any) => {
      if(head.id === id) {
        head.timeRequest = Date.now()
        const stopLists: any = [] 
        lists.map((res: any) => {
          if(!res) return res
            ...
            return res.Response.map((base: any) => {
              if(!base) return base
              if(head.stop_lists && head.stop_lists.filter((item: any) => item.ID_columns === base.ID_columns).length) return null
  ...
  updateStopListArr = (lists: any = []): any =>  clearStopListsArr(lists)

  /** Иммутабельное обновление выбранных данных пользователя */
  identifyUserInfo = (prevHeads: any, serverData: any, user: any, id: any): any => {
    return prevHeads = prevHeads.map((head: any) =>  {
      if(head.id === id) {
        head.timeRequest = Date.now()
        head.identifyInfo = {
  ...
  updateIdentifyInfo = (serverData: any): any =>  ({
    ...serverData,
    html: serverData.html ? htmlTransform(serverData.html) : "" 
  })

  /** Иммутабельное обновление выбранных данных пользователя */
  updateSelectedInfo = (prevHeads: any, user: any, id: any): any => {
    return prevHeads = prevHeads.map((head: any) =>  {
      if(head.id === id) {
        // head.timeRequest = Date.now()
        head.selectedInfo = user
  ...
  historySelectedInfo = ({prevHeads, historyIdentify, storeSelInfoFl}: any): any => {
    if(historyIdentify && historyIdentify.length) {
        historyIdentify.map((item: any) => {
          if(item.type === "Request") {
            prevHeads.map((head: any) => {
              if(
                (
  ...
  updateDocuments = (prevDocs: any, serverBlob: any, xhdoc: any): any => {
    return prevDocs = prevDocs.map((doc: any) =>  {
      if(doc.xhdoc === xhdoc) {
        doc.blob = serverBlob
      }
      return doc
  ...
  /** Иммутабельное обновление выбранных данных пользователя */
  updateSelectedUserInfo = (prevHeads: any, user: any, id: any, serverData: any ): any => {
  ...
  croinformInfoResponse = (serverData: any): any => ({
    html: serverData.html ? htmlTransform(serverData.html) : "", 
  ...
  updatedFsspInfo = (prevHeads: any, fssp: any, id: any ): any => {
    return prevHeads = prevHeads.map((head: any) =>  {
      if(head.id === id) {
        head.timeRequest = Date.now()
  ...
  clearedFsspInfo = (fssp: any): any =>  fssp ? htmlTransformFssp(fssp) : ""

  managementSource = (inputData: any): any => {
    const cloneFieldsArr = cloneDeep(fieldsArr);
    const fullOrganistionInfo = cloneFieldsArr.map((item: any) => {
      for (const el in inputData) {
        if(item.id === el )  return assign(item, { "data" : inputData[el]})
      }
      return item
    })
    const filteredManagementInfo = fullOrganistionInfo.filter((item: any) => (
      item.id === "befenicials" || 
      item.id === "founders_fl" || 
      item.id === "founders_ul" || 
      item.id === "heads"  || 
  ...
  getHeadsSrc = (inputData: any): any => {
    ...
    return fullOrganistionInfo.filter((item: any) => ( item.id === "heads" ))
  }

  updateManagmentSrc = (prevStore: any, serverData: any): any => {
    try {
      ...
      if( serverData.founders_fl.length ) {
        let notFoundsHeadsFl: any = []
        const updatedHeads = prevStore.get("heads").map((heads: any) => {
          if( serverData.founders_fl.filter((founders_fl: any) => heads.fio === founders_fl.fio).length) {
            const founders_fl = serverData.founders_fl.filter((founders_fl: any) => heads.fio === founders_fl.fio)[0]
            return {
              ...heads,
              position: concat(
                heads.position,
        ...
        serverData.founders_fl.map((founders_fl: any) => {
          if(!prevStore.get("heads").filter((head: any) => head.fio === founders_fl.fio).length) {
      ...
      if(serverData.share_holders_fl.length) {
        let notFoundsShareHoldersFl: any = []
        const headPerson = prevStore.get("heads").map((heads: any) => {
          if(serverData.share_holders_fl.filter((share_holders_fl: any) => heads.fio === share_holders_fl.fio).length) {
            const newInfo = serverData.share_holders_fl.filter((share_holders_fl: any) => heads.fio === share_holders_fl.fio)[0]
            return {
              ...heads,
              position: concat(
                heads.position,
        ...
        serverData.share_holders_fl.map((share_holders_fl: any) => {
          if(!prevStore.get("heads").filter((head: any) => head.fio === share_holders_fl.fio).length) {
  ...
  updateManagmentULSrc = (prevHeads: any, serverData: any, user: any): any => {
    ...
    if(serverData.heads_fl.length) {
      let notFoundsHeadsFl: any = []
      const headPerson = prevHeads.map((heads: any, i: any) => {
        if( serverData.heads_fl.filter((heads_fl: any) => heads.fio === heads_fl.fio).length) {
          const updatedHead = serverData.heads_fl.find((heads_fl: any) => heads.fio === heads_fl.fio)
          return {
            ...heads,
            position: concat(
              heads.position,
      ...
      serverData.heads_fl.map((heads_fl: any) => {
        if(!prevHeads.filter((heads: any) => heads.fio === heads_fl.fio).length) {
          return notFoundsHeadsFl.push({
            id: uuid(),
    ...
    // Учредители
    if(serverData.founders_fl.length) {
      let notFoundsFoundersFl: any = []
      const updatedHeads = prevHeads.map((heads: any) => {
        if( serverData.founders_fl.filter((founders_fl: any) => heads.fio === founders_fl.fio).length) {
          const founders_fl = serverData.founders_fl.find((founders_fl: any) => heads.fio === founders_fl.fio)
          return {
            ...heads,
            position: concat(
              heads.position,
      ...
      serverData.founders_fl.map((founders_fl: any) => {
        if(!prevHeads.filter((heads: any) => founders_fl.fio === heads.fio).length) {
    ...
    if(serverData.share_holders_fl.length) {
      let notFoundsShareHoldersFl: any = []
      const headPerson = prevHeads.map((heads: any) => {
        if(serverData.share_holders_fl.filter((share_holders_fl: any) => heads.fio === share_holders_fl.fio).length) {
          const share_holders_fl = serverData.share_holders_fl.find((share_holders_fl: any) => heads.fio === share_holders_fl.fio)
          return {
            ...heads,
            position: concat(
              heads.position,
      ...
      serverData.share_holders_fl.map((share_holders_fl: any) => {
        if(!prevHeads.filter((head: any) => head.fio === share_holders_fl.fio).length) {
          return notFoundsShareHoldersFl.push({
            id: uuid(),
            ActualDate: share_holders_fl.date ? share_holders_fl.date : getDate(Date.now()),
            fio: share_holders_fl.fio,
  ...
  riskSource = (inputData: any): any => {
    const cloneFieldsArr = cloneDeep(fieldsArr);
    const riskOrgInfo = cloneFieldsArr.map((item: any) => {
      for (const key in inputData) {
        if(item.id === key ) {
          return {
            id: key,
  ...
  updateRiskFactors = (props: any): any => {
    let { prevHeads, factors, id, digest, info } = props
    if(!factors.history || factors.history.length === 0) return prevHeads
    const newHeads = prevHeads.map((head: any) => {
      if(head.id === id) {
  ...
  saveRiskFaktorsFl = (props: any): any => {
    let { factors, id, heads } = props
    const headInfo = heads.find((item: any) => item.id === id)

    const digets = clearRiskFactors(factors.digest, true).map((item: any) => {
    ...
    const history = clearRiskFactors(factors.digest).map((item: any) => {
      item.object = `${headInfo.fio} (${headInfo.inn})`
      item.objectKey = id
      item.name = headInfo.fio
      item.key = item.hasOwnProperty("key") ? item.key : uuid()
  ...
  saveRiskFaktorsUl = (props: any): any => {
    ...
    const history = clearRiskFactors(factors.history).map((item: any) => {
      item.object = `${info.name} (${info.inn})`
      item.name = info.name
      item.key = item.hasOwnProperty("key") ? item.key : uuid()
      item.sortedTime = item.familia ? 
  ...
  updateDigets = (inputData: any, info: any, heads: any): any => {
    const newDigets = inputData.digest.map((item: any) => {
      item.object = `${info.name} (${info.inn})`
      item.name = info.name
      item.key = item.hasOwnProperty("key") ? item.key : uuid()
      return compactObj(item)
    })
    const newHistory = inputData.history.map((item: any) => {
      item.object = `${info.name} (${info.inn})`
      item.name = info.name
      item.key = item.hasOwnProperty("key") ? item.key : uuid()
      return compactObj(item)
  ...
  getHeadsRiskFactors = (heads: any, mainDigets: any): any => {
    let { digets=[], history=[] }: any = mainDigets
    heads.map((item: any) => {
      if(item.hasOwnProperty('risk_factors') && item.risk_factors.digets.length) {
        digets = concat(digets, item.risk_factors.digets.map((factor: any) => {
      ...
      if(item.hasOwnProperty('risk_factors') && item.risk_factors.history.length) {
        history = concat(history, item.risk_factors.history.map((factor: any) => {
          factor.object = `${item.fio} (${item.inn})`
          factor.name = item.fio
          factor.vremya_akceptovaniya_zapisi = factor.time
          factor.comment = factor.risk1
  ...
  _arbiterTransform = (item: any): any => {
    return item = [{
      key: '1',
      name: 'Истец',
  ...
  getUuid = (arr: any): any => arr.map((item: any) => {
    item.key = uuid()
    return item
  })
  ...
  updateEbgHeads = (props: any): any => {
    let { prevStore, managment, prevSelected } = props
    const newHeads =  managment.map((head: any) => {
      const checkRes = head.identityDocType === "NON_RESIDENT"
      const Region = checkRes ? 
        head.actualAddress.code_region ? 
          region.find((item: any) => item.title.toUpperCase().indexOf(getRegionTitle(head.actualAddress.region).toUpperCase()) !== -1) : 
          region.find((item: any) => item.value === head.actualAddress.code_region) :
            head.passport.address.region ? 
              head.passport.address.code_region ? 
                region.find((item: any) => item.value === head.passport.address.code_region) : 
                region.find((item: any) => item.title.toUpperCase().indexOf(getRegionTitle(head.passport.address.region).toUpperCase()) !== -1) : null
      const newHead: any = {
  ...
  getEbgFoundersUlKey = (props: any): any => {
    if(!props.length) return []
    else return props.map((item: any) => {
      item.key = uuid()
      return item
    })
  }
  ...
  updateEbgUlComSrc = (prevStore: any, serverData: any): any => {
    try {
      fieldsArr.map((fieldItem: any) => {
        for (const key in serverData) {
          if( fieldItem.search === "Predecessor" && fieldItem.func ) {
  ...
  /** Sls utils */
  customStopListSearch = (white: any = [], black: any = [], lists: any = []): any => {
    white.map((item1lvl: any) => {
      const {rows, ...all} = item1lvl
      rows.map((item2lvl: any) => {
        lists.push({ ...all, ...item2lvl, key: uuid(), search_request: "white"})
        return item2lvl
      })
      return item1lvl
    })
    black.map((item1lvl: any) => {
      const {rows, ...all} = item1lvl
      rows.map((item2lvl: any) => {
