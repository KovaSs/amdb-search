...
export const createReportFl = (props: any): string => {
  ...
  const renderDigets =(arr: any, text: any = "") => {
    arr.map(item => {
  ...
  const renderArrInfo = (arr: any, text: any = "") => {
    arr.map(item => text = text.concat(`<div>${item}</div>`))
    return text
  }

  const renderSL = (SL: any) => SL.map((item: any) => {
    const renderRowsItem = (list: any, i: any, text: any = "") => {
    ...
    return (
      `<div>
        ...
        ${ item.rows.map((list: any, i: any) => `<div> ${ renderRowsItem(list, i) } </div>` ) }
      </div>`
    )
  })
  return `<!DOCTYPE html>
...
export const createReportUl = (props: any): string => {
  const { 
    info, 
    digets,
    risks,
  ...
  const renderHeadsFl =(arr: any, text: any = "") => {
    text = text.concat(`
      <tr style="background-color: #d3d3d338;">
        <th>ФИО</th>
        <th>ИНН</th>
    ...
    arr.map(item => {
      let position = ""
      item.position.length &&  item.position.map((pos: any) => {
  ...
  const renderHeadsFlInfo = (arrHeads: any = [], headsContent: any = '<h2>Результаты проверки ФЛ:</h2>') => {
    arrHeads.map(head => {
  ...
  const renderNavMenu = (arrHeads: any = [], headsNavContent: any = '') => {
    arrHeads.map(head => {
  ...
  const renderNavMenuUl = (arrHeads: any = [], headsNavContent: any = '') => {
  ...
  const companyRiskSource = (arrRiskSrc: any = [], riskSrcContent: any = "<div class='divider'>") => {
    arrRiskSrc.map(item => {
      const {data: itemData} = item
      const renderArrInfo = (info: any) => {
  ...
  const renderFoundersUl = (arrFoundersUl: any = [], foundersRSContent: any = "<div class='divider'>") => {
    if(arrFoundersUl.length) {
      ...
      arrFoundersUl.map(founder => {
        if(companyUlRes.has(founder.key) && founder.key !== mainKey) {
          const founderRiskSrc = companyUlRes.get(founder.key).filter((value: any, key: any) => 
            key === "arbiter" ||
            key === "fns" ||
            key === "sanctions" ||
