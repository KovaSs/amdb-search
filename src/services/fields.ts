...
export const fieldsArr: any = [
  {search: "ShortNameRus", id: "name", title: "Сокрашеное наименование", data: "", func: (item: any) => item.replace(/\\/g, "")},

  {search: "FullNameRus", id: "full_name", title: "Полное наименование", data: "", func: (item: any) => item.replace(/\\/g, "")},

  {search: "INN", id: "inn", title: "ИНН", data: ""},

  {search: "OGRN", id: "ogrn", title: "ОГРН", data: ""},

  {search: "type", id: "company_type", title: "Тип компании", data: "", func: (item: any) => {
    try {
  ...
  {search: "DateFirstReg", id: "registration_date", title: "Дата регистрации", data: "", func: (item: any = 'Данные отсутствуют') => getDate(item)},

  {search: "Status", id: "status", title: "Статус", data: "", func: (item: any) => {
    try {
      if(!item) return 'Данные отсутствуют'
  ...
  {search: "OKVED2List", id: "okved", title: "Основной ОКВЭД", data: "", func: (item: any) => {
    try {
      if(!item) return ''
      if(Array.isArray(item.OKVED)) {
        const okved = item.OKVED.filter((el: any) =>  el.IsMain === 'true')
  ...
  {search: "ChangesInNameAndLegalForm", id: "previous_name", title: "Изменения в наименовании и организационно-правовой форме", data: "", func: (item: any) => {
    try {
      if(!item) return ''
      if(Array.isArray(item.Change)) {
        return item.Change.map((prevName: any) =>  {
  ...
  {search: "CompanySizeStr", id: "company_size", title: "Размер компании", data: "", func: (item: any) => {
    try {
      if(!item) return 'Данные отсутствуют'
      else if(item === " / ") return ""
      else return item
  ...
  {search: "LegalAddresses", id: "address", title: "Юридический адрес", data: "", func: (item: any) => {
    try {
      if(!item) return 'Данные отсутствуют'
      const { Address } = item
  ...
  {search: "PreviousAddress", id: "previous_address", title: "Предыдущие адреса", data: "", func: (item: any) => {
    try {
      if(!item) return 'Данные отсутствуют'
      const { Address } = item
      if(Array.isArray(Address)) {
        const addressArr = Address.map((address: any) => `${getDate(address.ActualDate)} / ${address.Address}`)
        return addressArr
      }
      else return Address.Address
    } catch (error) {
  ...
  {search: "StaffNumberFTS", id: "workers_range_fns", title: "Численность персонала по данным ФНС", data: "", func: (item: any) => {
    try {
      if(!item) return 'Данные отсутствуют'
      if(Array.isArray(item.Number)) {
        const workersArr = item.Number.map((workers: any) => `${getDate(workers.ActualDate)} / ${workers.content}`)
        return workersArr
      } else {
  ...
  {search: "CharterCapital", id: "capital", title: "Уставной капитал", func: (item: any) => {
    try {
      if(!item) return 'Данные отсутствуют'
  ...
  {search: "PaymentIndexStr", id: "payment_index", title: "Индекс платежной дисциплины", data: "", func: (item: any) => {
    try {
      if(!item) return 'Данные отсутствуют'
      else if(item === " / ") return ""
      else return item
  ...
  {search: "Predecessor", id: "precessors", title: "Предшедственники", data: "", func: (item: any) => {
    try {
      if(!item) return ''
      if(Array.isArray(item)) {
        return item.map((el: any) => `${el.Name} / ${el.INN} ${el.Status && el.Status.Text ? `/ ${el.Status.Text}` : ''}`)
      } else {
  ...
  {search: "Successor", id: "successors", title: "Преемники", data: "", func: (item: any) => {
    try {
      if(!item) return ''
      if(Array.isArray(item)) {
        return item.map((element: any) => `${element.Name} / ${element.INN} ${element.Status.Text ? `/ ${element.Status.Text}` : ''}`)
      } else {
        const { INN, Name, Status } = item
  ...
  {search: "PhoneList", id: "phone_list", title: "Список телефонов", data: "", func: (item: any) => {
    try {
      if(!item) return 'Данные отсутствуют'
      return item.Phone
    } catch (error) {
  ...
  {search: "arbitrazh", id: "arbiter", title: "Арбитраж", data: "", func: (item: any) => {
    try {
      if(!item) return ''
      const arbiter = {
        "istec":{
  ...
  {search: "fns", id: "fns", title: "ФНС", data: "", func: (item: any) => {
    try {
      if(!item) return ["Данные отсутствуют"]
      return item
    } catch (error) {
  ...
  {search: "IncludeInList", id: "spark_spiski", title: "Спарк. Списки", data: "", func: (item: any) => {
    try {
      if(!item) return ''
      if(Array.isArray(item.ListName)) {
        const sparkLists = item.ListName.map((item: any) => `${item.Id} / ${item.content}`)
        return sparkLists
      } else {
  ...
  {search: "PersonsWithoutWarrant", id: "heads", title: "Руководители", data: "", func: (item: any) => {
    try {
      if(!item) return ''
      if(Array.isArray(item.Person)) {
        const heads: any = []
        item.Person.map((elem: any) =>  {
  ...
  {search: "LeaderList", id: "leaders_list", title: "Состав руководителей", data: "", func: (item: any) => {
    try {
      if(!item) return ''
      if(Array.isArray(item.Leader)) {
        const heads: any = []
        item.Leader.map((elem: any) =>  {
  ...
  {search: "Sex", id: "sex", title: "Пол", data: "", func: (item: any) => {
...
export const fieldsArrIP: any = [
  ...
  {search: "DateReg", id: "registration_date", title: "Дата регистрации", data: "", func: (item: any = 'Данные отсутствуют') => item},
  ...
  {search: "OKATO", id: "address", title: "Юридический адрес", data: "", func: (item: any) => {
    if(!item) return 'Данные отсутствуют'
  ...
  {search: "LinkedOGRNIP", id: "previous_name", title: "Изменения в наименовании и организационно-правовой форме", data: "", func: (item: any) => {
    try {
      if(!item) return ''
      else if( Array.isArray(item.OGRNIP)) {
        return item.OGRNIP.map((item: any) => {
