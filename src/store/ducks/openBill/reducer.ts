...
export const openBillReducer = (state = new ReducerRecord(), action) => {
  ...
  switch (type) {
    ...
    case constants.LOAD_COMPANY_INFO_FAIL:
      return state
        .setIn(['requestLoading', 'companyMainInfoUpdate'], false)
    ...
    case constants.GET_AFFILATES_UL_SUCCESS:
      return state
    ...
    case constants.GET_IDENTIFY_USER_SUCCESS:
      return state
        ...
        .setIn(['errors', 'identifyUser', action.loading], false) 
    case constants.GET_IDENTIFY_USER_FAIL:
      return state
    ...
    case constants.GET_RISK_FACTORS_FL_INFO_START:
      return state
        .setIn(['requestLoading', 'getRiskFactorsFl', action.loading], true)
        .setIn(['errors', 'getRiskFactorsFl', action.loading], false)
    case constants.GET_RISK_FACTORS_FL_INFO_SUCCESS:
      return state
        .setIn(['riskFactors', action.loading], action.riskFactors)
        .setIn(['requestLoading', 'getRiskFactorsFl', action.loading], false)
        .setIn(['errors', 'getRiskFactorsFl', action.loading], false) 
    case constants.GET_RISK_FACTORS_FL_INFO_FAIL:
      return state
        .setIn(['requestLoading', 'getRiskFactorsFl', action.loading], false)
        .setIn(['errors', 'getRiskFactorsFl', action.loading], true)

    // Добавление нового риск фактора в дайджест лист
    case constants.ADD_RISK_FACTOR_FL_IN_DIGEST_LIST_START:
      return state
        .setIn(['requestLoading', 'addRistFactorInDigestList'], true)
        .setIn(['errors', 'addRistFactorInDigestList'], false)
    case constants.ADD_RISK_FACTOR_FL_IN_DIGEST_LIST_SUCCESS:
      return state
        .setIn(['riskFactors', action.loading], action.riskFactors)
        .setIn(['requestLoading', 'getRiskFactorsFl', action.loading], false) 
        .setIn(['requestLoading', 'addRistFactorInDigestList'], false)
        .setIn(['errors', 'addRistFactorInDigestList'], false) 
    case constants.ADD_RISK_FACTOR_FL_IN_DIGEST_LIST_FAIL:
      return state
        .setIn(['requestLoading', 'addRistFactorInDigestList'], false)
        .setIn(['errors', 'addRistFactorInDigestList'], true)

    // Редактирование риск-фактора из дайджест листа
    case constants.EDIT_RISK_FACTOR_FL_IN_DIGEST_LIST_START:
      return state
        .setIn(['requestLoading', 'deleteRistFactorInDigestList'], true)
        .setIn(['errors', 'deleteRistFactorInDigestList'], false)
    case constants.EDIT_RISK_FACTOR_FL_IN_DIGEST_LIST_SUCCESS:
      return state
        .setIn(['riskFactors', action.loading], action.riskFactors)
        .setIn(['requestLoading', 'getRiskFactorsFl', action.loading], false)
        .setIn(['requestLoading', 'deleteRistFactorInDigestList'], false)
        .setIn(['errors', 'deleteRistFactorInDigestList'], false) 
    case constants.EDIT_RISK_FACTOR_FL_IN_DIGEST_LIST_FAIL:
      return state
        .setIn(['requestLoading', 'deleteRistFactorInDigestList'], false)
        .setIn(['errors', 'deleteRistFactorInDigestList'], true)

    // Удаление риск фактора из дайджест листа
    case constants.DELETE_RISK_FACTOR_FL_IN_DIGEST_LIST_START:
      return state
        .setIn(['requestLoading', 'deleteRistFactorInDigestList'], true)
        .setIn(['errors', 'deleteRistFactorInDigestList'], false)
    case constants.DELETE_RISK_FACTOR_FL_IN_DIGEST_LIST_SUCCESS:
      return state
        .setIn(['riskFactors', action.loading], action.riskFactors)
        .setIn(['requestLoading', 'getRiskFactorsFl', action.loading], false)
        .setIn(['requestLoading', 'deleteRistFactorInDigestList'], false)
        .setIn(['errors', 'deleteRistFactorInDigestList'], false) 
    case constants.DELETE_RISK_FACTOR_FL_IN_DIGEST_LIST_FAIL:
      return state
        .setIn(['requestLoading', 'deleteRistFactorInDigestList'], false)
        .setIn(['errors', 'deleteRistFactorInDigestList'], true)

    // Получение данных о вхождении в стоплисты ЮЛ кампании
    case constants.GET_STOP_LISTS_UL_INFO_START:
      return state
        .setIn(['requestLoading', 'getStopListsUl'], true)
        .setIn(['errors', 'getStopListsUl'], false)
    case constants.GET_STOP_LISTS_UL_INFO_SUCCESS:
      return state
        .setIn(['companyResponse', 'stop_list'], payload)
        .setIn(['stopLists', action.loading], payload)
        .setIn(['requestLoading', 'getStopListsUl'], false)
        .setIn(['errors', 'getStopListsUl'], false) 
    case constants.GET_STOP_LISTS_UL_INFO_FAIL:
      return state
        .setIn(['requestLoading', 'getStopListsUl'], false)
        .setIn(['errors', 'getStopListsUl'], true)

    // Загрузка списка приложенных документов
    case constants.GET_DOCUMENTS_START:
      return state
