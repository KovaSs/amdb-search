...
class SearchCompanyInput extends PureComponent {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (errors && errors !== prevProps.errors && errors.status)
          ...
          <Form.Item style={{ marginRight: "1rem", top: -5 }}>
            {getFieldDecorator("data", {
              ...
              rules: [
                {
                  required: true,
                  message: "Строка поиска не должна быть пустой!",
                },
                {
                  pattern: "^[0-9]{10,15}$",
                  message: "Поисковой запрос должен состоять из 10-15 цифр!",
                },
              ],
            })(
              <Search
                ...
                enterButton={
                  showInfo ? (
                    ebgInn ? (
                      <Button className="search-btn" type="default">
                        {" "}
                        Вернуться в ЭБГ{" "}
                      </Button>
                    ) : (
  ...
  render() {
    ...
    if (error) return (
      <div style={{ textAlign: "center" }}>
        {
          'Ошибка в работе компонента "openBill -> SearchCompanyInput", пожалуйста перезагрузите страницу'
        }
      </div>
    );
    ...
    return (
      <Affix
        offsetTop={-8}
        onChange={(affixed) => this.changeCollapced(affixed)}
      >
        <Form
